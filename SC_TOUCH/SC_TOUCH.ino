...
void IRAM_ATTR touchIsr12() {
  Interrupt* intr = &interruptList[12];
  unsigned long now = millis();
  if (intr->fired + 300 < now) {
    intr->count++;
  }
  intr->fired = now;
}
...
void mySetup() {
  touchAttachInterrupt(12, touchIsr12, 30);

  displayBuffer[2] = "SC_TOUCH";
  updateDisplay();
}
